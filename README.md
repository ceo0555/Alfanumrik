...
When proxying through Vercel Functions, configure both `BACKEND_URL` (e.g. `https://backend.example.com`) and `SERVICE_TOKEN`; the helper automatically forwards the token in the `x-service-token` header on every request. The backend rejects requests that do not supply the correct service token when `SERVICE_TOKEN` is set.
